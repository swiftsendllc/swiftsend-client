...
export default function SettingsPage() {
  ...
  const yourAccounts = [
    {
      label: 'Accounts',
      leftIcon: <Image width={30} height={30} src={user.avatarURL!} alt="avatar" style={{ width: 30, height: 30 }} />,
      ...
      <Box sx={{ mb: 5, mt: 2, p: 2, width:"100%", overflowY:"scroll" }}>
        <Stack mt={1} mb={3}>
          ...
          <Autocomplete
            ...
            sx={{ width: '100%' }}
            renderInput={(params) => (
              <TextField {...params} slotProps={{ input: { sx: { borderRadius: '30px' } } }} label="Search" />
            )}
          />
        </Stack>
        <Divider />
        ...
        <List sx={{ width: '100%', mb: 1, padding: 0 }}>
          {yourAccounts.map((option, idx) => (
            <ListItemButton key={idx} sx={{ padding: 0, py: 1, borderRadius: 2 }}>
              <ListItemIcon sx={{ pr: 1, minWidth }}>{option.leftIcon}</ListItemIcon>
              {option.leftIcon ? (
        ...
        <List sx={{ width: '100%', mt: 1, mb: 2 }}>
          <ListItemButton sx={{ padding: 0, py: 1, borderRadius: 2 }}>
            ...
            <ListItemText primary="Dark mode" primaryTypographyProps={{ color: 'primary' }} />
          </ListItemButton>
        </List>
        <Divider />
        <List sx={{ width: '100%', mt: 1, mb: 2 }}>
          <ListItemButton sx={{ padding: 0, py: 1, borderRadius: 2 }} onClick={handleLogout}>
            <ListItemIcon sx={{ pr: 1, minWidth }}>
        ...
        <Stack width="100%">
          ...
          <Typography variant="body2" color="text.secondary" textAlign="center" mt={1} mb={6}>
            v1.0.0
          </Typography>
        </Stack>
      </Box>
